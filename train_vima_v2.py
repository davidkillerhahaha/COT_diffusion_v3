...
def train(params: argparse.Namespace):
    ...
    # vae = AutoencoderKL.from_pretrained('/home/nifei/sd-vae-ft-ema').to(device)
    ...
    # cemblayer = create_policy_from_ckpt("/home/rl/COT-diffusion/4M.ckpt", "cuda")
    cemblayer = create_policy_from_ckpt("/home/nifei/vima_policy_ckpt/4M.ckpt", "cuda")
    cemblayer.to(device)
    ...
    for epc in range(lastepc, params.epoch):
        ...
        with tqdm(dataloader, dynamic_ncols=True, disable=(local_rank % cnt != 0)) as tqdmDataLoader:
            for unnorm_image_dict, image_dict, traj_path, images_num in tqdmDataLoader:
                optimizer.zero_grad()
                unnorm_initial_image = unnorm_image_dict['0'].to(device)
                ...
                initial_image_token = cemblayer.obj_encoder.cropped_img_encoder(unnorm_initial_image).to(device)
                # print('initial_image_token:', initial_image_token.shape)

                cemb_list = torch.zeros([params.batchsize, params.vanilla_cdim], dtype=torch.float).to(device)
                ...
                for batch_id in range(params.batchsize):
                    ...
                    cemb_list[batch_id] = cemb.to(device) # (B, H)     (B, 256)
                # print('cemb_list:', cemb_list.shape)
                # print('initial_image_token:', initial_image_token.shape)
                condition_info = torch.concat([initial_image_token, cemb_list], dim=1).to(device)
                ...
                log_writer.add_scalar('Loss_1/train', loss_dict['0'].item(), epc)
                log_writer.add_scalar('Loss_2/train', loss_dict['1'].item(), epc)
                log_writer.add_scalar('Total_Loss/train', loss.item(), epc)
...
def main():
    ...
    parser.add_argument('--batchsize', type=int, default=2, help='batch size per device for training Unet model')
    parser.add_argument('--numworkers', type=int, default=8, help='num workers for training Unet model')
