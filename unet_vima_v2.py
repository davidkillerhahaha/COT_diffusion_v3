...
class EmbedSequential(nn.Sequential, EmbedBlock):
    def forward(self, x: torch.Tensor, temb: torch.Tensor, cemb: torch.Tensor) -> torch.Tensor:
        for layer in self:
            ...
            # print('layer in EmbedSequential:', layer)
...
class Unet(nn.Module):
    def __init__(self, in_ch=3, mod_ch=64, out_ch=3, ch_mul=[1, 2, 4, 8], num_res_blocks=2, cdim=1024, use_conv=True,
                 droprate=0, dtype=torch.float32, gdim=2):
        super().__init__()
        self.in_ch = in_ch
        self.mod_ch = mod_ch
        self.out_ch = out_ch
        ...
        # todo
        self.addtional_layer = nn.Sequential(
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, prompt_embedding: torch.Tensor) -> torch.Tensor:
        temb = self.temb_layer(timestep_embedding(t, self.mod_ch))
        cemb = prompt_embedding
