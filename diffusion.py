...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x_t:torch.Tensor, x_1:torch.Tensor, guid_num:torch.Tensor, t:torch.Tensor, cemb): #-> tuple[torch.Tensor, torch.Tensor]:
        """
    ...
    def p_sample(self, x_t,  x_1, guid_num,t, cemb) -> torch.Tensor:
        """
        ...
        mean, var = self.p_mean_variance(x_t , x_1, guid_num, t, cemb)
    ...
    def sample(self, shape:tuple, cemb, x_1, guid_num) -> torch.Tensor:
        """
        ...
        for _ in tqdm(range(self.T),dynamic_ncols=True, disable=(local_rank % torch.cuda.device_count() != 0)):
            tlist -= 1
            with torch.no_grad():
                x_t = self.p_sample(x_t, x_1, guid_num, tlist, cemb)
        x_t = torch.clamp(x_t, -1, 1)
        if local_rank == 0:
            print('ending sampling process...')
        return x_t
    
    def ddim_p_mean_variance(self, x_t:torch.Tensor,  x_1:torch.Tensor, guid_num:torch.Tensor, t:torch.Tensor, prevt:torch.Tensor, eta:float, **model_kwargs) -> torch.Tensor:
        """
        calculate the parameters of p_{theta}(x_{t-1}|x_t)
        """
        if model_kwargs == None:
    ...
    def ddim_p_sample(self, x_t:torch.Tensor,  x_1:torch.Tensor, guid_num:torch.Tensor, t:torch.Tensor, prevt:torch.Tensor, eta:float, **model_kwargs) -> torch.Tensor:
        if model_kwargs == None:
            model_kwargs = {}
        B, C = x_t.shape[:2]
        assert t.shape == (B,), f"size of t is not batch size {B}"
        mean, var = self.ddim_p_mean_variance(x_t , x_1, guid_num, t.type(dtype=torch.long), prevt.type(dtype=torch.long), eta, **model_kwargs)
        assert torch.isnan(mean).int().sum() == 0, f"nan in tensor mean when t = {t[0]}"
        assert torch.isnan(var).int().sum() == 0, f"nan in tensor var when t = {t[0]}"
        noise = torch.randn_like(x_t)
        noise[t <= 0] = 0 
    ...
    def ddim_sample(self, shape:tuple, x_1:torch.Tensor, guid_num:torch.Tensor, num_steps:int, eta:float, select:str, **model_kwargs) -> torch.Tensor:
        local_rank = get_rank()
        if local_rank == 0:
    ...
    def trainloss_loop(self, x_list, **model_kwargs):
        x_num = len(x_list)
        for i in range(x_num):
            if i == 0:      # first step generation without generated image input --> set the same as current image
                generated_image = x_list[0]
            else:
                generated_image = generated_image


    def trainloss_single(self, x_0:torch.Tensor, x_t_minus_1:torch.Tensor,  x_t_gt: torch.Tensor, **model_kwargs) -> torch.Tensor:
        '''
