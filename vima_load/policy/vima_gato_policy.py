...
class VIMAGatoPolicy(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.obj_encoder = vnn.GatoMultiViewRGBEncoder(
            emb_dim=embed_dim,
            views=["front", "top"],
            img_size=(64, 128),
            vit_patch_size=32,
        ...
        self._obj_xf_num_queries = self.obj_encoder.img_patch_len
    ...
    def forward(
        ...
        for q in range(self._obj_xf_num_queries):
            tokens[L_prompt + 1 + q :: self._obj_xf_num_queries + 1] = obs_token[
        ...
        seq_position_ids = any_stack(
            [
                torch.arange(
                    start=n_valids,
                    end=n_valids + L_obs * self._obj_xf_num_queries + L_action + 1,
                    dtype=torch.long,
                    device=self.device,
                )
                for n_valids in n_valid_prompt_tokens
        ...
        predicted_action_tokens = tokens_out[
            L_prompt + 1 + self._obj_xf_num_queries - 1 :: self._obj_xf_num_queries + 1
        ]
