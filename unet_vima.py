...
class Unet(nn.Module):
    def __init__(self, in_ch=3, mod_ch=64, out_ch=3, ch_mul=[1,2,4,8], num_res_blocks=2, cdim=11, use_conv=True, droprate=0, dtype=torch.float32, gdim=2):
        super().__init__()
        self.in_ch = in_ch
        self.mod_ch = mod_ch
        self.out_ch = out_ch
    ...
    def forward(self, x:torch.Tensor, guid_img: torch.Tensor, guid_num:torch.Tensor, t:torch.Tensor, cemb:torch.Tensor) -> torch.Tensor:
        temb = self.temb_layer(timestep_embedding(t, self.mod_ch))
        cemb = torch.cat((cemb,guid_num),-1)
        cemb = self.cemb_layer(cemb)
        #guidemb = self.img_layer(guid_img)
        hs = []
        h = x.type(self.dtype)
        ##TODO:: change here, use guided
        h = h + guid_img
