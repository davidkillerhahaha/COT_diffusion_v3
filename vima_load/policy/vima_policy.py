...
from ..utils import *
from vima_load import nn as vnn
...
class VIMAPolicy(nn.Module):
    def __init__(
        ...
        *,
        ...
        # # todo  额外加的
        # self.whole_image_encoder = vnn.MultiViewRGBEncoder(
        #     # img_size=(64, 128),
        #     img_size=(32, 64),
        #     emb_dim=embed_dim,
        #     views=["front", "top"],
    ...
    def forward(
        ...
        # print('obs_token:', obs_token, obs_token.shape)
        L_obs, B = obs_token.shape[:2]
        # print('L_obs:', L_obs, L_obs.shape)
        # print('B:', B, B.shape)
    ...
    def forward_prompt_assembly(self, prompts):
        ...
        # print("prompt_tokens before t5", prompt_tokens.size())
        ...
        # print("prompt_tokens after t5", prompt_tokens.size())
        return prompt_tokens, prompt_masks


    def forward_prompt_assembly_v2(self, prompts):
        raw_prompts_token_type, word_batch, image_batch, initial_obs_batch = prompts
        batch_word_emb = self.prompt_embedding(word_batch)
        batch_image_emb = self.obj_encoder(**image_batch)
        batch_image_emb = self.prompt_obj_post_layer(batch_image_emb)
        n_max_objs = batch_image_emb.shape[-2]
    ...
    def forward_initial_obs_token(self, initial_image):
        token = self.whole_image_encoder(initial_image)
        return token
