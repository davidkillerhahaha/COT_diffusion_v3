...
tokenizer = T5Tokenizer.from_pretrained('/home/nifei/t5-base')
...
def prepare_prompt(prompt: str, prompt_assets: dict):
# def prepare_prompt(prompt: str, prompt_assets: dict, la: int):
    views = ["front", "top"]
    views = sorted(views)
    #print("sss", prompt_assets)
    raw_prompt_token_types, word_batchs, image_batchs = [], [], []
#for i in range(len(prompt)):
    ...
    prompt_ids, prompt_tokens = encoding, tokens
    # print('prompt_ids:', prompt_ids, len(prompt_ids))
    # print('prompt_tokens:', prompt_tokens, len(prompt_tokens))
    # exit()
    ...
    for id, token in zip(prompt_ids, prompt_tokens):
        if token not in PLACEHOLDERS:
            ...
            #print("sss", asset["segm"]["obj_info"])
            obj_info = asset["segm"]["obj_info"]
            #print("sss", asset["placeholder_type"])
            placeholder_type = asset["placeholder_type"]   #103,106,109,115,116
            #if placeholder_type[0] == "object":
            if placeholder_type == "object":
                objects = [obj_info["obj_id"]]
            elif placeholder_type == "scene":
            #elif placeholder_type[0] == "scene":
            ...
            for view in views:
                ...
                for obj_id in objects:
                    #print(asset["segm"][view][la].size())
                    yxs = np.nonzero(segm_this_view == obj_id)
                    ys = yxs[0]
                    xs = yxs[1]
                    if len(xs) < 2 or len(ys) < 2:
                        continue
                    xmin, xmax = xs.min(), xs.max()#np.min(xs), np.max(xs)
                    ymin, ymax = ys.min(), ys.max()#np.min(ys), np.max(ys)
    ...
    '''
    raw_prompt_token_types.append(raw_prompt_token_type)
    word_batchs.append(word_batch)
